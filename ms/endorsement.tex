...
Applied to \citet{blair2013poverty}, where baseline support is 80\% and the treatment effect is $-1.1$ percentage points, the framework yields estimates ranging from 39\% (symmetric effects) to 99\% (extreme opponent reaction), with the binary switching model at 79\%. None of these estimates support their characterization of ``low regard.'' But the 79\% figure requires a strong behavioral assumption that is testable and likely violated in their data.
...
Binary switching has a strong testable implication. If all treatment group responses are either 0 or 1, the treatment group variance must equal $p(1-p)$. With $p = 0.789$, the predicted treatment variance is $0.789 \times 0.211 \approx 0.166$, and the standard deviation is approximately 0.41.

\citet{blair2013poverty} measured policy support on a 5-point scale rescaled to $[0,1]$. If the treatment group standard deviation is substantially below 0.41, the binary switching model is rejected. For reference, the control group standard deviation implied by their results is approximately 0.15 (roughly 10\% of a standard deviation corresponds to a 1.5 percentage point effect for the Afghan Taliban). A treatment standard deviation of similar magnitude would be inconsistent with binary switching by a factor of nearly three, indicating that respondents are not clustering at the extremes of the scale.
...
\Cref{tab:method_comparison} collects these estimates. The range is wide: from 39\% to 99\%. Baseline information narrows the range somewhat but cannot resolve the fundamental underidentification.
...
\citet{blair2013poverty} conducted endorsement experiments in Pakistan measuring attitudes toward the Afghan Taliban, Pakistani Taliban (TTP), Kashmiri militants, and sectarian organizations. Respondents were asked about support for benign policies (infrastructure development, polio vaccination) with and without militant group endorsements. The sample comprised approximately 6,000 respondents in a nationally representative survey.
...
\citeauthor{blair2013poverty}'s regression yields a constant of $\alpha = 0.80$ and treatment effect of $\beta = -0.011$, giving a treatment group average of 0.789. \Cref{tab:blair_reinterpretation} applies the binary switching model to each militant group.

\begin{table}[H]
\centering
\caption{Implied Support by Group Under Binary Switching}
\label{tab:blair_reinterpretation}
\input{../tabs/blair_reinterpretation.tex}
\end{table}

Under binary switching, implied support ranges from 78.5\% (Afghan Taliban) to 79.5\% (sectarian groups). Under symmetric effects with $d = 0.05$, these figures drop to roughly 35--45\%. The qualitative point holds across models: the data are not obviously consistent with ``low regard,'' but the precise level of support depends on which behavioral model one adopts.

\subsection{Why the Sign Alone is Insufficient}

The negative ATE tells us that more people switched away from the policy than toward it when the militant group endorsed it. Under every model considered, this means the net balance of switching tilts toward opposition. But with 80\% baseline support, even a slight negative tilt is consistent with majority support for the endorsing group under several plausible models.

The intuition is straightforward. Most respondents (80\%) support the policy in control. When the endorsement is introduced, some supporters of the militant group who did not previously support the policy now do, and some opponents of the militant group who previously supported the policy now withdraw. The net effect is slightly negative ($-1.1$ percentage points), meaning the second group slightly outweighs the first. But the first group could still be very large.
...
\citeauthor{blair2013poverty} measured responses on a 5-point scale rescaled to $[0,1]$. As noted in \cref{sec:framework}, binary switching predicts a bimodal treatment distribution concentrated at 0 and 1, with variance $p(1-p) \approx 0.166$. If the observed treatment variance is much smaller, binary switching is rejected as a descriptive model.

The authors do not report treatment group variances separately, but the overall standard deviation of approximately 0.15 (implied by their characterization of the Afghan Taliban effect as ``roughly 10\% of a standard deviation'') suggests a variance of about 0.023, an order of magnitude below the binary switching prediction. This is strong evidence against the binary model as a literal description of the data.

The binary switching result $p = \alpha + \beta$ should therefore be understood as the estimate \emph{conditional on a model that the data likely reject}, rather than as a direct reading of the data. It remains useful as one end of the range, but should not be reported as a point estimate.
...
With $q = 0.7$, $\beta = -0.011$, and $\alpha = 0.80$: $p = -0.011/0.7 + 0.80 = 0.784$. This yields 78.4\% support among opinion-holders, close to the full-sample binary switching estimate.

If instead we drop the baseline constraint and use only the ATE with the standard partial awareness formula $\text{ATE} = q(2p - 1)$, we get $p = \beta/(2q) + 0.5 = -0.011/1.4 + 0.5 = 49.2\%$. The discrepancy between 78.4\% and 49.2\% illustrates again how much baseline information matters, and how different auxiliary assumptions drive the results.
...
\caption{Implied support varies substantially across behavioral models, from 39\% (symmetric effects) to 99\% (extreme opponent reaction). No single estimate is privileged without a defended choice of model.}
...
The range of estimates---39\% to 99\%---is wide. This width is not a weakness of the analysis; it reflects the genuine underidentification of the problem. The data constrain the supporter proportion only loosely without a commitment to a specific behavioral model.
...
\citeauthor{blair2013poverty}'s characterization of ``low regard'' is not supported by any of the models considered here. Under every specification---including the one most favorable to their interpretation (symmetric effects with $d = 0.05$, yielding 39\%)---the implied support level is far from negligible. Under several plausible models, it exceeds 50\%. Their conclusion follows the sign of the ATE but not the magnitude of support it implies.

At the same time, the claim that ``79\% of Pakistanis support militant groups'' requires the binary switching model, which their own data likely reject on variance grounds. A more defensible summary is: the data are consistent with supporter proportions ranging from roughly 40\% to 80\%, depending on assumptions about individual switching behavior. The ATE sign tells us the net balance tips toward opposition, but the level of support could be anywhere in this range.
...
\textbf{Discuss identifying assumptions explicitly.} Moving from ATEs to support levels requires a behavioral model. Authors should state which model they assume, defend it, and report how sensitive results are to alternatives. The variance test described in \cref{sec:framework} provides a simple diagnostic for binary switching.
...
Finally, our analysis takes \citeauthor{blair2013poverty}'s reported estimates at face value. A full reanalysis with access to the microdata could test the variance implication directly and estimate models with heterogeneous switching.

\section{Conclusion}
\label{sec:conclusion}

\citet{blair2013poverty} conclude that Pakistanis hold militant groups in ``low regard'' based on small negative treatment effects. This inference is wrong. The sign of the ATE reflects the balance of switchers, not the level of support. Using both the treatment effect and baseline information, we derive a range of supporter proportions under different behavioral models: from 39\% (symmetric effects) to 99\% (extreme opponent reaction), with the binary switching model at 79\%.

No model considered here supports the characterization of ``low regard.'' But the precise level of support is sensitive to assumptions the data cannot adjudicate. The binary switching estimate of 79\% requires an assumption that the data likely reject on variance grounds. The symmetric effects estimate of 39\%, while lower, still represents a substantial minority rather than negligible support.
